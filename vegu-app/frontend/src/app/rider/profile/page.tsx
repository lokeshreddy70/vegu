...
export default function RiderProfilePage() {
  ...
  const initials = user.name ? user.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() : 'R';

  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Green header */}
      <div className="bg-veg px-4 pt-12 pb-8">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1.5 mt-1">
              ...
              <span className="text-white/80 text-xs capitalize">{partner?.status?.toLowerCase() ?? 'loading...'}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {user.phone && (
            <div className="px-4 py-3 flex items-center gap-3">
              <div className="w-8 h-8 bg-green-100 rounded-xl flex items-center justify-center">
                <Phone className="w-4 h-4 text-veg" />
              </div>
